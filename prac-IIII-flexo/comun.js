...
function initHandlers() {
    ...
    // TOUCH VARIABLES
    var lastTouchDistance = null; // for pinch-zoom tracking
    var lastTouchX, lastTouchY;   // for rotation tracking with touch
    ...
    canvas.addEventListener("touchstart", function(event) {
        if (event.touches.length === 1) {
            ...
            // Start pinch zoom, calculate the initial distance between two touches
            lastTouchDistance = getTouchDistance(event.touches[0], event.touches[1]);
        }
    }, false);

    // Handle touch move (rotation and zoom)
    canvas.addEventListener("touchmove", function(event) {
        if (event.touches.length === 1) {
            // Single touch: Rotate the object (similar to mousemove)
            var newTouchX = event.touches[0].clientX;
            var newTouchY = event.touches[0].clientY;

            myZeta -= (newTouchX - lastTouchX) * 0.005;
            myPhi  -= (newTouchY - lastTouchY) * 0.005;

            var margin = 0.01;
            myPhi = Math.min(Math.max(myPhi, margin), Math.PI - margin);
    ...
    // Button handlers and texture selectors stay the same
