...
function draw (model) {
  ...
  gl.bindBuffer(gl.ARRAY_BUFFER, model.idBufferVertices);
  switch(attributes) {
    case POSITION_NORMAL_TEXCOORDS:
      gl.vertexAttribPointer (program.vertexTexcoordsAttribute, 2, gl.FLOAT, false, nAttr*4, 6*4);
      gl.vertexAttribPointer (program.vertexNormalAttribute, 3, gl.FLOAT, false, nAttr*4, 3*4);
      gl.vertexAttribPointer (program.vertexPositionAttribute, 3, gl.FLOAT, false, nAttr*4, 0);
      break;
    case POSTION_NORMAL:
      gl.vertexAttribPointer (program.vertexNormalAttribute, 3, gl.FLOAT, false, nAttr*4, 3*4);
      gl.vertexAttribPointer (program.vertexPositionAttribute, 3, gl.FLOAT, false, nAttr*4, 0);
  ...
  for (var i = 0; i < botones.length; i++) {
    ...
    function(){
      switch (this.innerHTML) {
        ...
        case "ItalianaAgua": modeloSeleccionado = italianaAbajo; break;
      }
      requestAnimationFrame(drawScene);
    }, false);
  }
